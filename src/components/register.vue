	...
	<div>
		<section class="main">
        	...
        	<div class="login_form" >
	    		...
	    		<p>
					 <x-button @click.native="reg()" type="delete" class="submit_btn">绑定账号</x-button>
	    		</p>	    		
	    	</div>
      </section>
    </div>
...
export default {
	...
	data(){
		return {
            ...
            resend:false,
            time:60, //设置倒计时初始时间
            reg_url:this.url+'register',
	        send_code_url:this.url+'sendRegMsg',
	        code:'',
	...
	methods:{
        ...
        reg (){  //手机号验证码登录提交
            ...
            if(mobilePhone==""){
				...
				this.$http.post(this.reg_url,{mobilePhone:this.mobilePhone,verifyCode:verifyCode,code:this.code}).then((response)=>{
				    // 响应成功回调
					 if(response.data.status == "1"){
